...
class Game {
    ...
    constructor () {
        ...
        this.score = 0; // Atributo nuevo para reflejar la puntuación inicial
    }
    ...
    if (!this.ended) {
        ...
        this.render(); // Llamar a render para dibujar en el canvas
    }
}
    ...
    removeOpponent() {
        if (this.opponent) {
            document.body.removeChild(this.malo.png);
        }
        // Si la puntuación alcanza cierto umbral, introduce el jefe final
        if (this.score >= SOME_THRESHOLD) {
    ...
    endGame() {
        this.ended = true;
        const endImage = this.player.lives > 0 ? 'assets/you_win.png' : GAME_OVER_PICTURE;
        let gameOver = new Entity(this, this.width / 2, "auto", this.width / 4, this.height / 4, 0, endImage);
        gameOver.render();
    }
    /**
     * resetea el juego
